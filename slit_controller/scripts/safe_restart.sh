...
check_axis_moving() {
    ...
    result=$(echo "get:$axis:is_moving" | nc -U /tmp/slit_controller.sock 2>/dev/null)
...
if check_any_axis_moving; then
    ...
    systemctl restart slit-controller.service
    echo "Slit controller service restarted."
fi
