...
impl MultiAxis {
    ...
    async fn get_trid_client(&mut self) -> io::Result<Arc<Mutex<TcpStream>>> {
        if self.trid_client.is_none() {
            ...
            match TcpStream::connect(&SocketAddr::new(
                self.config.trid_ip.parse().unwrap(),
                self.config.trid_port,
            ))
            .await
            {
                Ok(stream) => {
    ...
    async fn get_rf256_client(&mut self) -> io::Result<Arc<Mutex<TcpStream>>> {
    ...
    #[instrument(skip(self))]
    async fn get_standa_client(&mut self, index: usize) -> io::Result<Arc<Mutex<TcpStream>>> {
        if self.standa_clients[index].is_none() {
            ...
            match TcpStream::connect(&SocketAddr::new(ip.parse().unwrap(), port)).await {
                Ok(stream) => {
    ...
    async fn get_axis(&mut self, index: usize) -> io::Result<&mut SingleAxis> {
            ...
            || self.rf256_client.is_none()
        {
            ...
            let rf256_client = self.get_rf256_client().await?;
            let standa_client = self.get_standa_client(index).await?;
            let trid_client = self.get_trid_client().await?;
    ...
    pub async fn get_velocity(&mut self, index: usize) -> io::Result<u32> {
        debug!("Getting velocity for axis {}", index);
        let axis = self.get_axis(index).await?;
        let velocity = axis.get_velocity();
        debug!("Got velocity {} for axis {}", velocity, index);
        Ok(velocity)
    }

    pub async fn set_velocity(&mut self, index: usize, velocity: u32) -> io::Result<()> {
    ...
    pub async fn get_acceleration(&mut self, index: usize) -> io::Result<u16> {
    ...
    pub async fn set_acceleration(&mut self, index: usize, acceleration: u16) -> io::Result<()> {
        debug!(
    ...
    pub async fn get_deceleration(&mut self, index: usize) -> io::Result<u16> {
    ...
    pub async fn set_deceleration(&mut self, index: usize, deceleration: u16) -> io::Result<()> {
        debug!(
    ...
    pub async fn get_position_window(&mut self, index: usize) -> io::Result<f32> {
    ...
    pub async fn set_position_window(
        &mut self,
        index: usize,
        position_window: f32,
    ) -> io::Result<()> {
    ...
    pub async fn get_time_limit(&mut self, index: usize) -> io::Result<Duration> {
    ...
    pub async fn set_time_limit(&mut self, index: usize, time_limit: Duration) -> io::Result<()> {
        debug!(
    ...
    pub async fn move_to_position(&mut self, index: usize, position: f32) -> Result<(), String> {
        ...
        let axis = self.get_axis(index).await.map_err(|e| {
            error!("Failed to get axis {}: {}", index, e);
            e.to_string()
        })?;

        let result = axis.move_to_position(position).await;
    ...
    pub async fn position(&mut self, index: usize) -> io::Result<f32> {
        ...
        let axis = self.get_axis(index).await?;
        let result = axis.position_with_retries(5).await;
        if let Err(ref e) = result {
    ...
    pub async fn temperature(&mut self, index: usize) -> io::Result<f32> {
        ...
        let axis = self.get_axis(index).await?;
        let result = axis.temperature().await;
        if let Err(ref e) = result {
