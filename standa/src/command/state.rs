...
impl<'a> StandaCommand<'a, 4> for StateParams {}

impl<'a> StandaGetSetCommand<'a, 4> for StateParams {
...
impl StateParams {
    pub fn is_moving(&self) -> bool {
        self.mv_cmd_sts.contains(MvCmdSts::RUNNING)
    }

    pub fn is_error(&self) -> bool {
        self.mv_cmd_sts.contains(MvCmdSts::ERROR)
    }

    pub fn left_switch(&self) -> bool {
        let gpio_flags = self.gpio_flags;
        gpio_flags.contains(GpioFlags::STATE_LEFT_EDGE)
    }

    pub fn right_switch(&self) -> bool {
        let gpio_flags = self.gpio_flags;
