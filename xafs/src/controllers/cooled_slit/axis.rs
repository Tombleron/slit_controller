...
impl CooledSlitAxis {
    ...
    pub async fn get_water_output_temperature(&self) -> Result<f32, String> {
        self.sensors_cs
            .get_temperature(self.axis as u8 + 4)
            .await
...
impl Axis for CooledSlitAxis {
    ...
    async fn get_attribute(&self, name: &str) -> anyhow::Result<f64> {
        match name {
            ...
            "water_output_temperature" => self
                .get_water_output_temperature()
                .await
                .map(|temp| temp as f64)
                .map_err(|err| {
